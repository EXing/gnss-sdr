...
DEFINE_double(e1_value_threshold,
0.3, "Value of the threshold for the acquisition");
DEFINE_int32(e1_value_CN0_dB_0,
50, "Value for the CN0_dB_0 in channel 0");
...
GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx_sptr
GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx_make(concurrent_queue<int> &queue);
...
GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx_sptr
GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx_make(concurrent_queue<int> &queue) {
    return GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx_sptr(
            new GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx(queue));
}
...
GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx::GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx(
        ...
        gr::block("GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx", gr::io_signature::make(0, 0, 0),
                  gr::io_signature::make(0, 0, 0)), channel_internal_queue(queue) {
    this->message_port_register_in(pmt::mp("events"));
    this->set_msg_handler(pmt::mp("events"), boost::bind(
            &GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx::msg_handler_events, this, _1));
    rx_message = 0;
}
...
std::shared_ptr <GNSSBlockInterface> acq_ = factory->GetBlock(config, "Acquisition",
                                                              "Galileo_E1_PCPS_QuickSync_Ambiguous_Acquisition", 1, 1);
...
std::shared_ptr <GNSSBlockInterface> acq_ = factory->GetBlock(config, "Acquisition",
                                                              "Galileo_E1_PCPS_QuickSync_Ambiguous_Acquisition", 1, 1);
acquisition = std::dynamic_pointer_cast<GalileoE1PcpsQuickSyncAmbiguousAcquisition>(acq_);
boost::shared_ptr <GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx> msg_rx = GalileoE1PcpsQuickSyncAmbiguousAcquisitionGSoC2014Test_msg_rx_make(
...
{
LOG(INFO)
<< "Start validation of results test";
...
->property("Acquisition.doppler_step", 125));
...
{
gnss_synchro.
PRN = 10; // This satellite is visible
}
else if (i == 1)
{
gnss_synchro.
PRN = 20; // This satellite is not visible
}
acquisition->
set_gnss_synchro(&gnss_synchro);
acquisition->
...
EXPECT_EQ((
unsigned int)1, correct_estimation_counter) << "Acquisition failure. Incorrect parameters estimation.";
}
else if (i == 1)
{
...
DLOG(INFO)
<< "End validation of results test";
}
...
{
LOG(INFO)
<< "Start validation of results with noise+interference test";
...
DLOG(INFO)
<< "End validation of results with noise+interference test";
}
...
acquisition->
set_doppler_step(config
->property("Acquisition.doppler_step", 500));
}) << "Failure setting doppler_step."<<
std::endl;
...
->property("Acquisition.threshold", 0.0));
}) << "Failure setting threshold."<<
std::endl;
