...
GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx_make(concurrent_queue<int> &queue);


class GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx : public gr::block {
private:
    friend GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx_sptr
    GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx_make(concurrent_queue<int> &queue);
...
GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx_make(concurrent_queue<int> &queue) {
    return GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx_sptr(
            new GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx(queue));
}
...
GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx::GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx(
        ...
        gr::block("GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx", gr::io_signature::make(0, 0, 0),
                  gr::io_signature::make(0, 0, 0)), channel_internal_queue(queue) {
    this->message_port_register_in(pmt::mp("events"));
    this->set_msg_handler(pmt::mp("events"),
                          boost::bind(&GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx::msg_handler_events, this,
                                      _1));
    rx_message = 0;
}
...
void GalileoE5aPcpsAcquisitionGSoC2014GensourceTest::process_message() {
    if (message == 1) {
        ...
        switch (sat) {
            case 0:
                delay_error_chips = std::abs((double) expected_delay_chips -
                                             (double) (gnss_synchro.Acq_delay_samples - 5) * 10230.0 /
                                             ((double) fs_in * 1e-3));
                doppler_error_hz = std::abs(expected_doppler_hz - gnss_synchro.Acq_doppler_hz);
                break;
            case 1:
                delay_error_chips = std::abs((double) expected_delay_chips1 -
                                             (double) (gnss_synchro.Acq_delay_samples - 5) * 10230.0 /
                                             ((double) fs_in * 1e-3));
                doppler_error_hz = std::abs(expected_doppler_hz1 - gnss_synchro.Acq_doppler_hz);
                break;
            case 2:
                delay_error_chips = std::abs((double) expected_delay_chips2 -
                                             (double) (gnss_synchro.Acq_delay_samples - 5) * 10230.0 /
                                             ((double) fs_in * 1e-3));
                doppler_error_hz = std::abs(expected_doppler_hz2 - gnss_synchro.Acq_doppler_hz);
                break;
            case 3:
                delay_error_chips = std::abs((double) expected_delay_chips3 -
                                             (double) (gnss_synchro.Acq_delay_samples - 5) * 10230.0 /
                                             ((double) fs_in * 1e-3));
...
long long int end = 0;
acquisition = std::make_shared<GalileoE5aNoncoherentIQAcquisitionCaf>(config.get(), "Acquisition", 1, 1);
boost::shared_ptr <GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx> msg_rx = GalileoE5aPcpsAcquisitionGSoC2014GensourceTest_msg_rx_make(
...
->property("Acquisition_Galileo.doppler_max", 10000));
...
->property("Acquisition_Galileo.doppler_step", 500));
...
->property("Acquisition_Galileo.threshold", 0.0));
