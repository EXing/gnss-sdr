...
rp2->
rinex_obs_header(rp2
->obsFile, eph, 0.0, bands);
rp2->obsFile.seekp(0);
no_more_finds = false;
while(!rp2->obsFile.
...
rp1->
rinex_obs_header(rp1
->obsFile, eph_gps, eph_gal, 0.0, "1B 5X");
rp1->obsFile.seekp(0);
int systems_found = 0;
...
gs4.
Pseudorange_m = 22000000;
gs4.
Carrier_phase_rads = 23.4;
gs4.
Carrier_Doppler_hz = 1534;
gs4.
CN0_dB_hz = 42;

gnss_pseudoranges_map.
insert( std::pair<int, Gnss_Synchro>(1, gs1));
gnss_pseudoranges_map.
insert( std::pair<int, Gnss_Synchro>(2, gs2));
gnss_pseudoranges_map.
insert( std::pair<int, Gnss_Synchro>(3, gs3));
gnss_pseudoranges_map.
insert( std::pair<int, Gnss_Synchro>(4, gs4));

rp->
log_rinex_obs(rp
->obsFile, eph, 0.0, gnss_pseudoranges_map);
rp->obsFile.seekp(0);
...
gs2.
Pseudorange_m = 22000002.1;
gs2.
Carrier_phase_rads = 45.4;
gs2.
Carrier_Doppler_hz = 321;
gs2.
CN0_dB_hz = 39;
...
rp->
log_rinex_obs(rp
->obsFile, eph_gps, eph_cnav, 0.0, gnss_pseudoranges_map);
rp->obsFile.seekp(0);
...
std::string expected_str(
        "G08  22000002.100 6         7.226 6       321.000 6        39.000  22000000.000 7         3.724 7      1534.000 7        42.000");
EXPECT_EQ(0, expected_str.
compare(line_aux)
);
...
gs3.
Pseudorange_m = 22000003.3;
gs3.
Carrier_phase_rads = 43.3;
gs3.
Carrier_Doppler_hz = -321;
gs3.
CN0_dB_hz = 40;
...
rp->
log_rinex_obs(rp
->obsFile, eph, 0.0, gnss_pseudoranges_map, bands);
rp->obsFile.seekp(0);
...
std::string expected_str(
        "E08  22000002.100 6         7.226 6       321.000 6        39.000  22000000.000 7         3.724 7      1534.000 7        42.000");
EXPECT_EQ(0, expected_str.
compare(line_aux)
);
...
Pseudorange_m = 22000000;
gs8.
Carrier_phase_rads = 0.8;
gs8.
Carrier_Doppler_hz = -20;
gs8.
CN0_dB_hz = 42;
...
insert( std::pair<int, Gnss_Synchro>(4, gs4));
gnss_pseudoranges_map.
insert( std::pair<int, Gnss_Synchro>(5, gs5));
gnss_pseudoranges_map.
insert( std::pair<int, Gnss_Synchro>(6, gs6));
gnss_pseudoranges_map.
insert( std::pair<int, Gnss_Synchro>(7, gs7));
gnss_pseudoranges_map.
insert( std::pair<int, Gnss_Synchro>(8, gs8));

rp->
log_rinex_obs(rp
->obsFile, eph_gps, eph_gal, 0.0, gnss_pseudoranges_map);
...
std::string expected_str(
        "E16  22000000.000 7         0.127 7       -20.000 7        42.000  22000000.000 6         8.292 6      1534.000 6        41.000");
EXPECT_EQ(0, expected_str.
compare(line_aux)
);
