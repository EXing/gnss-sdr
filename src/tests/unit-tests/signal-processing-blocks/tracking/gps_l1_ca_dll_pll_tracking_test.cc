...
GpsL1CADllPllTrackingTest_msg_rx::GpsL1CADllPllTrackingTest_msg_rx() :
        gr::block("GpsL1CADllPllTrackingTest_msg_rx", gr::io_signature::make(0, 0, 0),
...
int GpsL1CADllPllTrackingTest::configure_generator() {
    ...
    if (FLAGS_dynamic_position.empty()) {
        p2 = std::string("-static_position=") + FLAGS_static_position + std::string(",") +
             std::to_string(FLAGS_duration * 10);
    } else {
...
true_obs_file.
append(std::to_string(test_satellite_PRN));
true_obs_file.append(".dat");
ASSERT_NO_THROW({
if (true_obs_data.
open_obs_file(true_obs_file)
== false)
{
throw
...
std::shared_ptr<TrackingInterface> tracking = std::make_shared<GpsL1CaDllPllTracking>(config.get(), "Tracking_1C", 1,
                                                                                      1);
...
gnss_synchro.
Acq_delay_samples =
(GPS_L1_CA_CODE_LENGTH_CHIPS - true_obs_data.prn_delay_chips / GPS_L1_CA_CODE_LENGTH_CHIPS) * baseband_sampling_freq *
...
top_block->
connect(file_source,
0, gr_interleaved_char_to_complex, 0);
top_block->
...
ASSERT_NO_THROW({
if (trk_dump.
open_obs_file(std::string("./tracking_ch_0.dat"))
== false)
{
throw
...
trk_timestamp_s(epoch_counter) =
        static_cast<double>(trk_dump.PRN_start_sample_count) / static_cast<double>(baseband_sampling_freq);
...
trk_acc_carrier_phase_cycles = trk_acc_carrier_phase_cycles.subvec(initial_meas_point(0),
                                                                   trk_acc_carrier_phase_cycles.size() - 1);
...
std::cout <<  "Signal tracking completed in " << (end - begin) << " microseconds" <<
std::endl;
}
